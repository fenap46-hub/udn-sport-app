...
async function renderPlayers() {
  ...
  wrap.innerHTML = filtered.map(p => `
    <div class="player-row">
      <div class="top">
        <span class="name">${escapeHtml(p.name)}${p.isPendingPlayer ? '<span class="pending-tag">A PRUEBA</span>' : ''}</span>
        <span class="cat">${p.category}${p.status && p.registeredAt ? ' · ' + formatHora(p.registeredAt) : ''}</span>
      </div>
...
async function confirmAddPlayer() {
  ...
  document.getElementById('add-form').classList.remove('show');
  await renderPlayers();
  toast('Jugador agregado como "A PRUEBA". Quedara pendiente de validar al sincronizar.');
}
    ...
    return `
      <div class="list-item">
        ...
        <div class="row btn-block-gap">
          <button class="btn btn-ghost btn-sm" onclick="syncSingleList(${l.id})">
            <i class="ti ti-refresh"></i> Sincronizar
          </button>
          <button class="btn btn-danger-outline btn-sm" onclick="confirmDeleteList(${l.id})">
...
async function confirmDeleteList(listId) {
  ...
  const isSynced = list && list.syncStatus === 'synced';
  const warning = isSynced
    ? 'Esta lista ya esta sincronizada con Google Sheets (los datos ya estan seguros ahi). ¿Eliminarla del celular?'
    : 'Esta lista TODAVIA NO se ha sincronizado. Si la eliminas ahora, esos datos de asistencia se pierden para siempre. ¿Eliminarla de todas formas?';
...
async function deleteAllSyncedNow() {
  const n = await deleteAllSyncedLists();
  toast(n ? `${n} lista(s) sincronizada(s) eliminada(s)` : 'No hay listas sincronizadas para eliminar');
  await renderListas();
  await refreshAsisHomeCount();
}
...
function friendlyError(e) {
  ...
  if (msg === 'SIN_MARCAR') return 'no has marcado a ningun jugador todavia';
